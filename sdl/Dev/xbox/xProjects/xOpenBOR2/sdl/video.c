...
#else
		screen = SDL_SetVideoMode(videomodes.hRes*savedata.screen[videoMode][0],videomodes.vRes*savedata.screen[videoMode][0],16,savedata.fullscreen?(SDL_SWSURFACE|SDL_FULLSCREEN):(SDL_SWSURFACE));
#endif
		SDL_ShowCursor(SDL_DISABLE);
...
#else
		screen = SDL_SetVideoMode(videomodes.hRes,videomodes.vRes,8*bytes_per_pixel,savedata.fullscreen?(SDL_SWSURFACE|SDL_FULLSCREEN):(SDL_SWSURFACE));
#endif
		SDL_ShowCursor(SDL_DISABLE);
	}
...
void video_fullscreen_flip()
{
	...
	if(savedata.screen[videoMode][0])
		screen = SDL_SetVideoMode(w,h,16,savedata.fullscreen?(SDL_SWSURFACE|SDL_FULLSCREEN):(SDL_SWSURFACE));
	else
		screen = SDL_SetVideoMode(w,h,8*bytes_per_pixel,savedata.fullscreen?(SDL_SWSURFACE|SDL_FULLSCREEN):(SDL_SWSURFACE));
	SDL_ShowCursor(SDL_DISABLE);
	SDL_SetColors(screen,colors,0,256);
	if(bscreen) SDL_SetColors(bscreen,colors,0,256);
}
...
{
#ifdef XBOX
	int xpos, ypos, xstretch, ystretch;

	if(enable)
	{
		SDL_XBOX_GetScreenPosition(&xpos, &ypos);
		SDL_XBOX_GetScreenStretch(&xstretch, &ystretch);
		SDL_XBOX_SetScreenPosition(160, ypos);
		SDL_XBOX_SetScreenStretch(-320, ystretch);
	}
	else
	{
		XboxSetScreen();
